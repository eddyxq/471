<html>
	<link rel="stylesheet" type="text/css" href="main.css">
	<body>
		<center>
			<div class="content">
	  
				<h1><p><font size="7" color="#ffffff">Welcome Back Administrator</font></p></h1>

				<a class="button" href="underconstruction.php">Search User</a>
				<a class="button" href="addUser.php">Add User</a>
				<a class="button" href="underconstruction.php">Edit User Information</a>
				<a class="button" href="underconstruction.php">Remove User</a>
				<a class="buttonlogout" href="index.php">Logout</a>

			</div>
		</center>
	</body>
</html>